...
if ! command -v bats &>/dev/null; then
  BATS_DIR="$TESTS_ROOT/.bats-core"
  if [ ! -d "$BATS_DIR" ]; then
    echo "Installing bats-core..." >&2
    git clone --depth 1 https://github.com/bats-core/bats-core.git "$BATS_DIR" 2>&1 | tail -1
    git clone --depth 1 https://github.com/bats-core/bats-support.git "$TESTS_ROOT/.bats-support" 2>&1 | tail -1
    git clone --depth 1 https://github.com/bats-core/bats-assert.git "$TESTS_ROOT/.bats-assert" 2>&1 | tail -1
  fi
